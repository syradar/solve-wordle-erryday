...
function dateToIndex(e, a) {
  const s = new Date(e);
  const t = new Date(a).setHours(0, 0, 0, 0) - s.setHours(0, 0, 0, 0);
  return Math.floor(t / 864e5);
}

const originDate = new Date(2021, 5, 19, 0, 0, 0, 0);

function getIndex(todaysDate) {
  return dateToIndex(originDate, todaysDate);
}

function getSolution(index, wordList) {
  const wordListIndex = index % wordList.length;
